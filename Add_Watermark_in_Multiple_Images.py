...
class ImageWatermarkerApp(QMainWindow):
    ...
    def load_images(self):
        ...
        file_filter = "Image Files (*.png *.jpg *.jpeg *.bmp *.webp);;All Files (*)"
        file_names, _ = QFileDialog.getOpenFileNames(self, "Open Image Files", "", file_filter, options=options)
        if file_names:
    ...
    def start_watermarking(self):
        ...
        if not self.image_paths:
            QMessageBox.warning(self, "Error", "Please select images to watermark.")
            return
        ...
        if folder:
            ...
            self.watermark_thread = WatermarkThread(self.image_paths, self.watermark_path, folder, opacity)
